...
def trajectory(p, U, x0):
  ...
  return fori_loop(0, T, loop, (X, 0.0))
...
def ilqr(p, x0, U, tol=1e-6):
  ...
  def loop(state):
    ...
    U = U + dU
    X, J_new = trajectory(p, U, X[0] + dX[0]) #actual trajectory
    if (J_new < J_opt): #have to do something about regularization
  ...
  X, J_opt = trajectory(p, U, x0)
  X, U, J_opt= lax.while_loop(cond, loop, (X, U, J_opt))
...
def cilqr(cilqr_iterations, ilqr_iterations, c_spec, x0, U, t):
  ...
  return fori_loop(0, cilqr_iterations, loop, (U, t))
