...
def trajectory_cost(cost, X, U):
  """ Calculates cost of LQR trajectory"""

  def loop(t, total):
    return total + cost(t, X[t], U[t])

  return fori_loop(0, T, loop, 0)
...
def lqr_predict(spec, x0, alphas = jnp.array([1]), J_opt = 0):
  """ Using dynamics from LQR spec and K,k from lqr_solve, calculates optimal trajectory (forward pass) """
  T, control_dim, _ = spec.R.shape
  _, state_dim, _ = spec.Q.shape

  K, k = lqr_solve(spec) #backward pass


  #do a line search here 
  #instead of break, just do return 


  def fwd_loop(t, state):
    spec, X, U = state
    A, B = spec.A[t], spec.B[t]
    u = mv(K[t], X[t]) + alpha * k[t]
    x = mv(A, X[t]) + mv(B, u)
    X = jo.index_update(X, jo.index[t + 1], x)
    U = jo.index_update(U, jo.index[t], u)
    return spec, X, U

  def line_search(t, state):
    converged = state
    if (converged):
      return state

    _, X, U = fori_loop(0, T, fwd_loop, (spec, X, U, alphas[t])) #forward 
    new_state = lqr_converged()
  ...
  X = jo.index_update(X, jo.index[0], x0)
  _, X, U = fori_loop(0, alphas.size, line_search, (spec, X, U)) #for alpha in alphas

  return X, U

def lqr_converged(X, U):
...
def ilqr(iterations, p, x0, U, X=None):
  ...
  def loop(_, state):
    ...
    if (changed):
      J_opt = trajectory_cost(p_lqr, X, U)
      changed = False
    dX, dU = lqr_predict(p_lqr, jnp.zeros_like(x0), alphas, J_opt)
    U = U + dU
  ...
  changed = True
  return fori_loop(0, iterations, loop, (X, U, changed))
